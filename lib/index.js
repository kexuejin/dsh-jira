	...
	ctx.connection.rpc.handle(JIRA_RPC_CHANNEL, async (endpoint, payload) => {
		...
		try {
			switch (endpoint) {
				...
				case "getTransitions": return ok(await client.listTransitions(getTransitionsArgs(payload)));
...
async function syncIssues(ctx, config) {
...
async function writeBackSettledExecutions(ctx, config, seen) {
...
function registerJiraWorkBoardSync(ctx, config) {
	if (config.workBoardSync === false || clean(config.baseUrl) === void 0) return;
	const board = workBoard(ctx);
	if (board?.registerExternalTaskHandler !== void 0) ctx.effect(() => board.registerExternalTaskHandler("jira:", { async transition(taskId, input) {
		const issueKey = issueKeyFromTaskId(taskId);
		if (issueKey === void 0) throw new Error(`task ${taskId} is not a Jira issue`);
		return (await createJiraClient(ctx, config).transitionIssue({
			issueKey,
			...input.transitionId === void 0 ? {} : { transitionId: input.transitionId },
			...input.transitionName === void 0 ? {} : { transitionName: input.transitionName },
	...
	const runSync = () => {
		...
		syncInFlight = true;
		syncIssues(ctx, config).catch((error) => {
	...
	const runWriteback = () => {
		...
		writeBackSettledExecutions(ctx, config, seenWritebacks).catch((error) => {
			console.error("[dsh-jira] work-board writeback failed", error);
		}).finally(() => {
