...
var JiraClient = class {
	...
	async search(args) {
		...
		const total = numberField$2(response.total);
		return {
			jql,
...
function configPath(dir = join(dshHome(), "jira")) {
...
function authMode(value) {
	return value === "pat" || value === "basic" ? value : void 0;
}
...
function projectMappings(value) {
	...
	return value.flatMap((item) => {
		...
		return [{
			...
			...mode === void 0 ? {} : { mode },
			...mappedPermission === void 0 ? {} : { permission: mappedPermission }
		}];
	});
}
function sanitizeJiraConfigPatch(value) {
	...
	const timeoutMs = numberField$1(raw.timeoutMs);
	const maxResults = numberField$1(raw.maxResults);
	...
	const workBoardSyncIntervalMs = numberField$1(raw.workBoardSyncIntervalMs);
	...
	return {
		...baseUrl === void 0 ? {} : { baseUrl },
		...mode === void 0 ? {} : { authMode: mode },
		...tokenCredentialRef === void 0 ? {} : { tokenCredentialRef },
		...username === void 0 ? {} : { username },
		...strictTls === void 0 ? {} : { strictTls },
		...caCertificatePath === void 0 ? {} : { caCertificatePath },
		...proxyUrl === void 0 ? {} : { proxyUrl },
		...
		...workBoardSync === void 0 ? {} : { workBoardSync },
		...workBoardSyncJql === void 0 ? {} : { workBoardSyncJql },
		...workBoardSyncIntervalMs === void 0 ? {} : { workBoardSyncIntervalMs },
		...workBoardProjectMappings === void 0 ? {} : { workBoardProjectMappings },
		...workBoardWriteback === void 0 ? {} : { workBoardWriteback },
		...workBoardDoneTransition === void 0 ? {} : { workBoardDoneTransition },
		...workBoardFailedTransition === void 0 ? {} : { workBoardFailedTransition },
		...workBoardManualTransitions === void 0 ? {} : { workBoardManualTransitions }
	};
}
...
var JiraConfigStore = class {
	base;
	path;
	overrides;
	constructor(base, path = configPath()) {
		this.base = base;
		this.path = path;
	}
	load() {
		if (this.overrides !== void 0) return this.overrides;
		try {
...
function getTransitionsArgs(payload) {
	return { issueKey: stringField(payload, "issueKey") ?? "" };
}
...
function saveConfigArgs(payload) {
	return { config: sanitizeJiraConfigPatch(isRecord(payload) && isRecord(payload.config) ? payload.config : {}) };
}
function workBoardIntegration(ctx) {
	const service = ctx.get("workBoard");
	return typeof service?.syncManualTasks === "function" && typeof service.manualSnapshot === "function" ? { status: "connected" } : { status: "standalone" };
}
function saveCredentialArgs(payload) {
	return {
		credentialRef: stringField(payload, "credentialRef") ?? "",
		value: stringField(payload, "value") ?? ""
	};
}
async function saveCredential(ctx, payload) {
	...
	if (credentials === void 0) throw new Error("credentials service is not mounted");
	await credentials.set(ref, value);
	return {
		credentialRef: ref,
		message: `Saved credential ${ref}.`
	};
}
function ok(value) {
	return {
...
function registerJiraRpc(ctx, store) {
	ctx.connection.rpc.handle(JIRA_RPC_CHANNEL, async (endpoint, payload) => {
		try {
			switch (endpoint) {
				case "status": return ok({
					...await createJiraClient(ctx, store.current()).status(),
					workBoard: workBoardIntegration(ctx)
				});
				case "config": return ok(store.view());
				case "saveConfig": return ok(store.save(saveConfigArgs(payload).config));
				case "saveCredential": return ok(await saveCredential(ctx, saveCredentialArgs(payload)));
				case "search": return ok(await createJiraClient(ctx, store.current()).search(searchArgs(payload)));
				case "getIssue": return ok(await createJiraClient(ctx, store.current()).getIssue(getIssueArgs(payload)));
				case "getTransitions": return ok(await createJiraClient(ctx, store.current()).listTransitions(getTransitionsArgs(payload)));
				case "addComment": return ok(await createJiraClient(ctx, store.current()).addComment(addCommentArgs(payload)));
				case "transitionIssue": return ok(await createJiraClient(ctx, store.current()).transitionIssue(transitionIssueArgs(payload)));
...
//#endregion
//#region src/work-source.ts
function statusFor$1(jiraStatus) {
...
function currentConfig$1(input) {
	return typeof input === "function" ? input() : input;
}
function createJiraWorkSource(ctx, config) {
	const client = () => createJiraClient(ctx, currentConfig$1(config));
	return {
		id: "jira",
		title: "Jira",
		filters: [{
		...
		async list(query) {
			return (await client().search(query.filterId === "mine" ? {
				view: "assigned",
				jql: "assignee = currentUser() AND resolution = Unresolved ORDER BY updated DESC"
			} : { view: "assigned" })).issues.map((issue) => ({
				id: `jira:${issue.key}`,
				sourceId: "jira",
				externalId: issue.key,
				title: issue.summary,
				status: statusFor$1(issue.status),
				...issue.priority === void 0 ? {} : { priority: issue.priority },
				...issue.updated === void 0 ? {} : { updatedAt: issue.updated },
				...issue.url === void 0 ? {} : { url: issue.url }
			}));
		},
		async get(id) {
			...
			return {
				...
				status: statusFor$1(detail.status),
				...detail.priority === void 0 ? {} : { priority: detail.priority },
				...detail.updated === void 0 ? {} : { updatedAt: detail.updated },
				...detail.url === void 0 ? {} : { url: detail.url },
				...detail.description === void 0 ? {} : { description: detail.description },
		...
		async actions(item) {
			return [{
				id: "start",
				label: "Start work",
				prompt: item.prompt ?? `Work on Jira ${item.externalId}.`
			}, ...item.url === void 0 ? [] : [{
				id: "open",
				label: "Open in Jira",
				prompt: `Open ${item.url}`
			}]];
		}
	};
}
/** Register the Jira source when a work-board service is present. */
function registerJiraWorkSource(ctx, config) {
	const workBoard = ctx.get("workBoard");
	if (workBoard === void 0) return;
	const dispose = workBoard.register(createJiraWorkSource(ctx, config));
	ctx.effect(() => dispose, "jira: work-board source");
}
//#endregion
//#region src/work-board-sync.ts
function currentConfig(input) {
	return typeof input === "function" ? input() : input;
}
const SOURCE_ID = "jira";
const DEFAULT_SYNC_INTERVAL_MS = 3e5;
...
function issueProjectKey(issueKey) {
	return normalizeMappingKey(issueKey.split("-", 1)[0] ?? issueKey);
}
function mappingFor(issue, config) {
...
function issueDescription(issue) {
	return [
		`Jira: ${issue.key}`,
		`Status: ${issue.status}`,
		issue.priority === void 0 ? void 0 : `Priority: ${issue.priority}`,
		issue.issueType === void 0 ? void 0 : `Type: ${issue.issueType}`,
		issue.assignee === void 0 ? void 0 : `Assignee: ${issue.assignee}`,
		issue.reporter === void 0 ? void 0 : `Reporter: ${issue.reporter}`,
		issue.updated === void 0 ? void 0 : `Updated: ${issue.updated}`,
		`URL: ${issue.url}`
...
function taskFromIssue(issue, now, config) {
	...
	return {
		...
		updatedAt,
		executions: [],
		...mapping?.workspaceId === void 0 ? {} : { workspaceId: mapping.workspaceId },
		...mapping?.mode === void 0 ? {} : { mode: mapping.mode },
		...mapping?.permission === void 0 ? {} : { permission: mapping.permission }
	};
}
function issueKeyFromTaskId(id) {
	return /^jira:([A-Z][A-Z0-9]+-\d+)$/u.exec(id)?.[1];
}
function latestSettledExecution(task) {
	return [...task.executions].filter((execution) => execution.endedAt !== void 0 && execution.result !== void 0).sort((a, b) => (b.endedAt ?? b.startedAt) - (a.endedAt ?? a.startedAt))[0];
}
function writebackMarker(executionId) {
	return `${WRITEBACK_MARKER_PREFIX}${executionId}]`;
}
function writebackBody(task, execution) {
	const outcome = execution.result ?? "unknown";
	return [
		`${writebackMarker(execution.id)} DSH work-board agent execution ${outcome}.`,
		execution.sessionId === void 0 ? void 0 : `Session: ${execution.sessionId}`,
		execution.error === void 0 ? void 0 : `Error: ${execution.error}`
	].filter((line) => line !== void 0).join("\n");
}
function workBoard(ctx) {
	const service = ctx.get("workBoard");
	return typeof service?.syncManualTasks === "function" && typeof service.manualSnapshot === "function" ? service : void 0;
}
async function syncIssues(ctx, input) {
	const config = currentConfig(input);
	if (config.workBoardSync === false || clean(config.baseUrl) === void 0) return;
...
function transitionMarker(executionId) {
	return `${WRITEBACK_MARKER_PREFIX}${executionId}:transition]`;
}
async function writeBackSettledExecutions(ctx, input, seen) {
	const config = currentConfig(input);
	if (config.workBoardWriteback === false || clean(config.baseUrl) === void 0) return;
	const board = workBoard(ctx);
	if (board === void 0) return;
	const client = createJiraClient(ctx, config);
	for (const task of board.manualSnapshot().tasks) {
		const issueKey = issueKeyFromTaskId(task.id);
		if (issueKey === void 0) continue;
		const execution = latestSettledExecution(task);
		if (execution === void 0) continue;
		const marker = writebackMarker(execution.id);
		if (seen.has(marker)) continue;
		const transitionName = execution.result === "failed" ? clean(config.workBoardFailedTransition) : execution.result === "succeeded" ? clean(config.workBoardDoneTransition) : void 0;
		try {
			...
			await client.addComment({
				issueKey,
				body: writebackBody(task, execution)
			});
			seen.add(marker);
			const transitionMarkerValue = transitionMarker(execution.id);
			if (transitionName !== void 0 && !detail.comments.some((comment) => comment.body.includes(transitionMarkerValue))) try {
				await client.transitionIssue({
					issueKey,
					transitionName,
					comment: writebackBody(task, execution)
				});
			} catch (error) {
...
function registerJiraWorkBoardSync(ctx, input) {
	const board = workBoard(ctx);
	if (board?.registerExternalTaskHandler !== void 0) ctx.effect(() => board.registerExternalTaskHandler("jira:", { async transition(taskId, transitionInput) {
		const issueKey = issueKeyFromTaskId(taskId);
		if (issueKey === void 0) throw new Error(`task ${taskId} is not a Jira issue`);
		return (await createJiraClient(ctx, currentConfig(input)).transitionIssue({
			issueKey,
			...transitionInput.transitionId === void 0 ? {} : { transitionId: transitionInput.transitionId },
			...transitionInput.transitionName === void 0 ? {} : { transitionName: transitionInput.transitionName },
			...transitionInput.comment === void 0 ? {} : { comment: transitionInput.comment }
		})).message;
	} }), "jira: work-board external transition handler");
	const seenWritebacks = /* @__PURE__ */ new Set();
	let syncInFlight = false;
	let writebackInFlight = false;
	const runSync = () => {
		if (syncInFlight) return;
		syncInFlight = true;
		syncIssues(ctx, input).catch((error) => {
	...
	const runWriteback = () => {
		...
		writeBackSettledExecutions(ctx, input, seenWritebacks).catch((error) => {
			console.error("[dsh-jira] work-board writeback failed", error);
		}).finally(() => {
			writebackInFlight = false;
		});
	};
...
const Config = z.object({
	...
	workBoardSyncIntervalMs: z.number().step(1).min(3e4).default(3e5),
...
//#endregion
export { Config, JIRA_RPC_CHANNEL, apply, createJiraClient, createJiraWorkSource, inject, internals, name, registerJiraRpc, registerJiraWorkSource, resolveConfig };
